...
let showingOnlyWallpaper = false;
const wallpapers = ["1.png", "2.jpg", "3.png"];
...
function toggleWallpaperOnly() {
  const mainElement = document.querySelector("main");
  const wallpaperButton = document.getElementById("wallpaper-switcher");

  if (!showingOnlyWallpaper) {
    // Hide all content except the wallpaper
    mainElement.style.display = "none";
    if (wallpaperButton) wallpaperButton.style.display = "block";
    showingOnlyWallpaper = true;
  } else {
    // Show the content again
    mainElement.style.display = "block";
    showingOnlyWallpaper = false;
  }
}
...
function cycleWallpaper() {
  ...
  document.body.style.backgroundImage = `url("../assets/bg/${nextWallpaper}")`;
  // Save the current wallpaper preference
  setLocalStorageItem("manifest_wallpaper", currentWallpaperIndex);
}

function createWallpaperButton() {
  const button = document.createElement("button");
  button.setAttribute("id", "wallpaper-switcher");
...
function addThemeToggleOption() {
  const hiddenButton = document.createElement("button");
  hiddenButton.style.display = "none";
  hiddenButton.setAttribute("id", "theme-toggle-hidden");
  hiddenButton.addEventListener("click", toggleTheme);
...
function onLoad() {
  ...
  // Create the wallpaper switcher button
  createWallpaperButton();

  // Add hidden theme toggle button to keep function used
  addThemeToggleOption();

  // Load saved wallpaper preference if any
  const savedWallpaperIndex = getLocalStorageItem("manifest_wallpaper");
  if (savedWallpaperIndex !== null && !isNaN(parseInt(savedWallpaperIndex))) {
    currentWallpaperIndex = parseInt(savedWallpaperIndex);
    const wallpaper = wallpapers[currentWallpaperIndex];
    document.body.style.backgroundImage = `url("../assets/bg/${wallpaper}")`;
  }
